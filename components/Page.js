...
export default function Page(props) {
  ...
  async function createLangMenu(currentLang, fileName) {
    const resEN = await fetch(
      `https://aurt-data-4i1quklt5-oxygen-cat.vercel.app/static/locales/en/navitems.json`
    )
    const resCA = await fetch(
      `https://aurt-data-4i1quklt5-oxygen-cat.vercel.app/static/locales/ca/navitems.json`
    )
    const resES = await fetch(
      `https://aurt-data-4i1quklt5-oxygen-cat.vercel.app/static/locales/es/navitems.json`
    )
    ...
    async function updateContentFromFileName(fileName) {
      ...
      if (lang !== newLang || content.length == 0) {
        const res = await fetch(
          `https://aurt-data-4i1quklt5-oxygen-cat.vercel.app/static/locales/${newLang}/navitems.json`
        )
